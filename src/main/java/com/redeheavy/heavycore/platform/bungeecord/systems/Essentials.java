...
public class Essentials {
    ...
    public Essentials (@NonNull Plugin instanciator) {
    ...
    public File essentialConfig(String name) {
        ...
        try {
            ...
            Logger.send(Level.ERROR, LoggerType.BUNGEECORD, "Houve um erro ao criar um dos arquivos de configuração.");
            e.printStackTrace();
        }
        return resourceFile;
    }
    ...
    public Configuration getConfig(String file) {
        try {
            ...
            Logger.send(Level.ERROR, LoggerType.BUNGEECORD, "Houve um erro ao carregar ums dos arquivos de configuração: " + file + ".yml para o plugin: " + instantiator.getDescription().getName() + ".");
        }
        return null;
    }
